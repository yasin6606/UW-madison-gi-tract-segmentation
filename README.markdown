...
### 4.4. Train
The model is fine-tuned on the UW-Madison dataset using PyTorch. The training pipeline includes:
1. Loading the preprocessed three-channel images and corresponding segmentation masks.
2. Applying data augmentations (random flips, rotations, and brightness adjustments) using the `albumentations` library.
...
Training is performed on a GPU to accelerate computation, with a batch size of 16 and a maximum of 50 epochs. The EfficientNet-B1 encoder is initialized with ImageNet weights, and all layers are fine-tuned to adapt to the medical imaging domain.

### 4.5. Evaluate
The model is evaluated using the competition’s primary metric: the **multi-label Dice coefficient**, which measures segmentation accuracy for the stomach, small intestine, and large intestine. The Dice coefficient is computed per class and averaged across all images in the test set. The evaluation pipeline involves:
...
The model achieves a competitive Dice score on the validation set, with further evaluation on the Kaggle test set submitted via the competition platform. Results are analyzed to identify segmentation challenges, such as small intestine boundaries, for future improvements.
...
| <span style="color: red">torchmetrics</span> | <span style="color: red">1.2.0</span>  |
